...
Endpoints:
  - GET  /api/health     → System status & API key check
  - GET  /api/skills     → List generated skill manifests
  - GET  /api/skills/{name} → Get a specific skill manifest content
  - POST /api/discover   → Run full Discovery pipeline (ingest → council → SKILLS.md)
  - POST /api/chat       → Skill-aware chat (RAG + Skill Manifest)
  - DELETE /api/skills/{name} → Delete a skill manifest
"""
...
tags_metadata = [
    ...
    },
    {
        ...
        "description": (
            ...
            '"hat" the AI can wear — e.g., "staking", "authentication", "payments".'
        ),
    },
    {
        "name": "Discovery",
...
app = FastAPI(
    ...
    description=(
        ...
        "---\n\n"
        "### 🚀 Quick Start\n\n"
        "1. **Check health**: `GET /api/health` — Verify your API keys are set\n"
        "2. **Add data**: Place code in `data/code/`, docs in `data/docs/`\n"
        "3. **Run discovery**: `POST /api/discover` — Generate a Skill Manifest\n"
        "4. **Chat**: `POST /api/chat` — Ask questions using your new Skill Hat\n\n"
        "---\n\n"
        "### 📖 Terminology\n\n"
        "| Term | Meaning |\n"
        "|------|--------|\n"
        ...
        "| **Discovery** | The pipeline that ingests code → generates SKILLS.md |\n"
    ),
    version="0.1.0",
    docs_url="/docs",
    redoc_url="/redoc",
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [
                {
                    "skill_name": "staking",
                    "content": "# STAKING — Skill Manifest\n\n## Overview\n...",
                    "exists": True,
                }
            ]
        }
...
@app.get(
    ...
    description=(
        ...
        "**Required API keys:**\n"
        "- `GOOGLE_API_KEY` — Needed for all LLM operations\n\n"
        "**Optional API keys:**\n"
        "- `GITHUB_TOKEN` — For live GitHub repo ingestion (otherwise uses local `data/code/`)\n"
        "- `CONFLUENCE_URL` + `CONFLUENCE_TOKEN` — For live Confluence page ingestion"
    ),
)
...
@app.post(
    ...
    description=(
        ...
        "### ⏱️ Expected Duration: 60–90 seconds\n\n"
        "### Pipeline Steps:\n\n"
        "| Step | Action | Details |\n"
        "|------|--------|---------|\n"
...
@app.post(
    ...
    description=(
        ...
        "### Request Body:\n"
        "- `query`: Your question\n"
        "- `skill_hat`: Which skill to use (must exist — run Discovery first)\n"
...
async def startup_event() -> None:
    ...
    print()
    print("  API Keys:")
    print(f"    Google Gemini:  {'✅ Ready' if api_keys['google'] else '❌ MISSING (required!)'}")
    print(f"    GitHub Token:   {'✅ Ready' if api_keys['github'] else '⬜ Not set (using local files)'}")
    print(f"    Confluence:     {'✅ Ready' if api_keys['confluence'] else '⬜ Not set (using local files)'}")
    print()
    print("  Data Folder:")
